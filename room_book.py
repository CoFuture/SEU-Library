...
def searchId(session, id, invideID):
    ...
    a = '$' + session.get(
        f'http://10.9.4.215/ClientWeb/pro/ajax/data/searchAccount.aspx?type=&ReservaApply=ReservaApply&term={id}').json()[
        0]['id'] + ','
    for i in invideID:
        a += (session.get(
            f'http://10.9.4.215/ClientWeb/pro/ajax/data/searchAccount.aspx?type=&ReservaApply=ReservaApply&term={i}').json()[
            0]['id']) + ','
    return a[:-1]
    ...
    for i in requests.get(url1).json()['data']:  # 研讨间
        ...
        for time1 in time_list:  # 每一个期望时间
            ...
            for ii in i['ts']:  # 当前研讨间已有的预约
                ...
                # 判断当前研讨间时间是否可以预约
                if ((start1 - int(time1[0])) * (end1 - int(time1[0])) >= 0) and (
                        (start1 - int(time1[1])) * (end1 - int(time1[1])) >= 0) and (not start1 == int(time1[0])) and (
                        not end1 == int(time1[1])):
                    continue
                else:
                    flag = 0
                    break
